...
24. **No level names, no flavour text.** Decorating levels with names or
    quips is developer voice leaking into the product — virtually no shipped
    puzzle game does it, and players don't read it. On-board text exists only
    to teach a new rule the first time it appears; every other level shows
    nothing, and the text bar hides entirely when empty. Absence is the genre
    norm. (Generated-content corollary: a pipeline makes it *easy* to stamp
    autogenerated names and captions on everything — resist; ease of adding
    text is not a reason it should exist.)
